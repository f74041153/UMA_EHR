...
public class AlbumService {
    ...
    public static final String SCOPE_EHR_VIEW = "EHR:view";
    public static final String SCOPE_EHR_DELETE = "EHR:delete";
    ...
    public Response create(Album newAlbum) {
        System.out.println(newAlbum.getPatientId());
        System.out.println(newAlbum.getTime());
        System.out.println(newAlbum.getDetail());
        
        Principal userPrincipal = request.getUserPrincipal();

        newAlbum.setId(UUID.randomUUID().toString());
        newAlbum.setDoctorId(userPrincipal.getName());
      //  newAlbum.setUserId("48c1364b-2f20-4d3d-bb86-d6e9731d54dc");
        ...
        queryDuplicatedAlbum.setParameter("userId", "48c1364b-2f20-4d3d-bb86-d6e9731d54dc");
    ...
    public Response findAll() {
       // System.out.println(this.entityManager.createQuery("from Album where userId = :id").setParameter("id", request.getUserPrincipal().getName()));
        return Response.ok(this.entityManager.createQuery("from Album where patientId = :id").setParameter("id", request.getUserPrincipal().getName()).getResultList()).build();
    }
    ...
    private void createProtectedResource(Album album) {
        try {
            ...
            scopes.add(new ScopeRepresentation(SCOPE_EHR_VIEW));
            scopes.add(new ScopeRepresentation(SCOPE_EHR_DELETE));

            ResourceRepresentation albumResource = new ResourceRepresentation(album.getId(), scopes, "/EHR/" + album.getId(), "http://photoz.com/EHR");

            albumResource.setOwner(album.getPatientId());
